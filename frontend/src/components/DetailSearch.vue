      ...
      <div v-else class="detail-page-content">
        ...
        <div class="detail-content-wrapper">
          ...
          <div class="detail-info">
            <h2 class="cat-name">{{ cat.nama_kucing }}</h2>
            
            <div class="info-grid">
              <div class="info-row">
                <span class="label">Status</span>
                <span class="value"><span class="status-badge">{{ cat.status }}</span></span>
              </div>
              
              <div class="info-row">
                <span class="label">Pelapor</span>
                <span class="value">{{ cat.nama_pelapor || '-' }}</span>
              </div>
...
/* ✅ MODAL BOX BERWARNA COKLAT (MEOWMENT) */
.modal-box {
    background-color: var(--browny, #8B5E3C);
    border-radius: 20px;
    padding: 0; 
    width: 95%; /* Lebih lebar di layar kecil */
    max-width: 1200px; /* ✅ SEBELUMNYA 950px, SEKARANG 1200px */
...
/* Header Buttons (Di atas kartu putih) */
.action-buttons {
...
/* ✅ KARTU PUTIH DI DALAM (WRAPPER) */
.detail-content-wrapper {
    background-color: white; /* Kartu Putih */
    border-radius: 16px;
    padding: 30px;
...
.photo-main {
    width: 100%; 
    height: 500px; /* ✅ SEBELUMNYA 380px, SEKARANG 500px BIAR LEBIH PUAS LIHATNYA */
    border-radius: 12px; 
    overflow: hidden;
    background: #f9f9f9; 
...
.cat-name {
    font-size: 2.2rem; margin: 0 0 10px 0; color: var(--dark); font-family: 'Poppins', sans-serif;
}

.info-grid {
    display: flex; flex-direction: column; gap: 12px;
    /* background: #fbfbfb; padding: 0; border: none; */
}
.info-row {
    display: grid; grid-template-columns: 110px 1fr; align-items: flex-start;
}
.info-row .label {
    font-weight: 600; color: #888; font-size: 0.95rem;
}
.info-row .value {
    font-weight: 500; color: #333; line-height: 1.5;
}

/* Tags */
.tags-list { display: flex; flex-wrap: wrap; gap: 6px; }
.mini-tag {
    background: #e0d4cc; color: #5d4037; padding: 4px 12px; border-radius: 15px; font-size: 0.8rem; font-weight: 600;
}

/* Description */
.description-box {
    margin-top: 20px; line-height: 1.6; color: #555; 
    padding-top: 20px; border-top: 1px dashed #eee;
}

/* Status Badge */
.status-badge {
    background: #ffe3c7; color: #8d5e46; padding: 4px 10px; border-radius: 6px; font-weight: bold; text-transform: uppercase; font-size: 0.85rem;
}

/* Responsive */
/* --- RESPONSIVE MOBILE --- */
@media (max-width: 768px) {
    .modal-overlay { padding: 10px; } /* Space tipis di pinggir */
    .modal-box { max-height: 95vh; }
    .detail-page-content { padding: 16px; }
    ...
    .photo-main {
        height: 300px; /* Kurangi tinggi foto di HP */
    }

    .cat-name { font-size: 1.6rem; }

    /* Info Row jadi numpuk (Label di atas value) biar gak sempit */
    .info-row {
    ...
    .action-buttons {
        position: sticky; top: 0; z-index: 5; /* Biar tombol back gampang diraih */
    }
}
</style>